...
import { saveProject, type SaveProjectIssue } from "@/app/(admin)/admin/actions";
import { LocalizedInput, LocalizedTextarea } from "@/components/admin/localized-field";
...
const inputClass = "w-full rounded-lg border border-ink/15 bg-card px-3 py-2 text-sm text-ink";
...
function buildPayload(input: ProjectInput): ProjectInput {
  ...
  return {
    ...
    caseStudy: {
      ...
      delivered: cs.delivered.map((d) => ({ ...d, detail: undefinedIfBlank(d.detail) })),
      gallery: cs.gallery.map((g) => ({ ...g, caption: undefinedIfBlank(g.caption) })),
    },
  };
}
...
export function ProjectForm({ project }: { project: Project | null }) {
  ...
  function updateCaseStudy<K extends keyof CaseStudy>(key: K, next: CaseStudy[K]) {
    setValue((v) => ({ ...v, caseStudy: { ...(v.caseStudy ?? emptyCaseStudy()), [key]: next } }));
  }
    ...
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-8 pb-24">
          ...
          <ul className="list-disc space-y-1 pl-5">
            {issues.map((issue, i) => (
              <li key={i}>
                <button
                  type="button"
                  onClick={() => scrollToField(document.getElementById(fieldId(issue.path)))}
      ...
      <section className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Type" error={ferr("typeKey")}>
            <select
              id={fieldId(["typeKey"])}
              value={value.typeKey}
              onChange={(e) => update("typeKey", e.target.value as ProjectInput["typeKey"])}
        ...
        <ImageUpload
          ...
          altError={{ en: ferr("cover", "alt", "en"), sr: ferr("cover", "alt", "sr") }}
          captionError={{ en: ferr("cover", "caption", "en"), sr: ferr("cover", "caption", "sr") }}
        />
      </section>
          ...
          <input
            ...
            onChange={(e) =>
              setValue((v) => ({
                ...
                caseStudy: e.target.checked ? v.caseStudy ?? emptyCaseStudy() : v.caseStudy,
              }))
            }
          />
        ...
        {value.hasCaseStudy && (
          <div className="space-y-6 rounded-xl border border-ink/10 p-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Client (optional)" error={ferr("caseStudy", "client")}>
                <input
                  id={fieldId(["caseStudy", "client"])}
                  value={cs.client ?? ""}
                  onChange={(e) => updateCaseStudy("client", e.target.value || undefined)}
                  className={inputClass}
                />
              </Field>
              <Field label="Status (optional)" error={ferr("caseStudy", "status")}>
                <select
                  id={fieldId(["caseStudy", "status"])}
                  value={cs.status ?? ""}
                  onChange={(e) =>
                    updateCaseStudy("status", (e.target.value || undefined) as ProjectStatus | undefined)
                  }
                  className={inputClass}
                >
                  <option value="">—</option>
              ...
              <Field label="Live URL (optional)" error={ferr("caseStudy", "url")}>
                <input
                  id={fieldId(["caseStudy", "url"])}
                  value={cs.url ?? ""}
                  onChange={(e) => updateCaseStudy("url", e.target.value || undefined)}
                  placeholder="https://…"
            ...
            <LocalizedInput
              ...
              error={{ en: ferr("caseStudy", "role", "en"), sr: ferr("caseStudy", "role", "sr") }}
            />
            <LocalizedInput
              ...
              error={{ en: ferr("caseStudy", "scope", "en"), sr: ferr("caseStudy", "scope", "sr") }}
            />
            ...
            >
              <Repeater
                ...
                newItem={() => ({ problem: { ...EMPTY_LOCALIZED }, solution: { ...EMPTY_LOCALIZED } })}
                ...
                renderItem={(item, i, upd) => (
                  <div className="space-y-2">
                    ...
                    <LocalizedTextarea
                      ...
                      error={{
                        en: ferr("caseStudy", "challenges", i, "solution", "en"),
                        sr: ferr("caseStudy", "challenges", i, "solution", "sr"),
                      }}
                    />
                  </div>
                )}
            ...
            >
              <Repeater
                ...
                newItem={() => ({ title: { ...EMPTY_LOCALIZED }, detail: undefined })}
                addLabel="Add item"
            ...
            >
              <Repeater
                ...
                newItem={() => ({ label: { ...EMPTY_LOCALIZED }, detail: { ...EMPTY_LOCALIZED } })}
                addLabel="Add phase"
            ...
            >
              <Repeater
                ...
                renderItem={(item, i, upd) => (
                  <div className="space-y-2">
                    <Field label="Value" error={ferr("caseStudy", "metrics", i, "value")}>
                      <input
                        ...
                        placeholder="e.g. 300+"
                        className={`${inputClass} ${
                          ferr("caseStudy", "metrics", i, "value") ? errorInputClass : ""
                        }`}
                      />
                    </Field>
                    <LocalizedInput
              ...
              <Repeater
                ...
                renderItem={(item, i, upd) => (
                  <div className="space-y-2">
                    ...
                    />
                    <Field label="Items" error={ferr("caseStudy", "stack", i, "items")}>
                      <CommaListInput
                        ...
                        className={`${inputClass} ${
                          ferr("caseStudy", "stack", i, "items") ? errorInputClass : ""
                        }`}
                      />
                    </Field>
                  </div>
            ...
            >
              <Repeater
                ...
                max={5}
                newItem={() => ({ src: "", width: 0, height: 0, alt: { ...EMPTY_LOCALIZED } })}
                addLabel="Add image"
            ...
            <LocalizedTextarea
              ...
              error={{ en: ferr("caseStudy", "note", "en"), sr: ferr("caseStudy", "note", "sr") }}
            />
          </div>
        )}
      </section>

      {error && issues.length === 0 && <p className="text-sm text-red-400">{error}</p>}
