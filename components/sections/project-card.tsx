...
function ProjectCardShell({ project, children }: { project: Project; children: React.ReactNode }) {
  return (
    <Link
  ...
  return (
    <ProjectCardShell project={project}>
      ...
      <div className="flex flex-1 flex-col px-[7px] pb-1.5 pt-4">
        <div className="text-lg font-semibold tracking-[-0.01em]">{project.title}</div>
        <div className="mt-[3px] text-sm text-muted">{t[project.typeKey]}</div>
        <p className="mt-2 line-clamp-2 text-sm text-muted">{pick(project.summary, lang)}</p>
